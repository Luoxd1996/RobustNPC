...
class NetworkTrainer(object):
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        from augmentation_invariant_transformation import augmentation_invariant_transformtion
        # train augmentation-invariant framework
        data_dict = next(data_generator)
        data = data_dict['data']
        target = data_dict['target']
        ...
        if self.fp16:
            with autocast():
                aug_one, aug_two, label_one = augmentation_invariant_transformtion(data, target)
                output1 = self.network(aug_one)
                output2 = self.network(aug_two)
                del data
                del aug_one
                del aug_two
                l = self.loss(output1, label_one) + self.loss(output2, label_one) + 0.1 * torch.mean((F.softmax(output1, 1) - F.softmax(output2, 1))**2)
